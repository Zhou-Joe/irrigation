...
class AISettings(models.Model):
    ...
    DEFAULT_SYSTEM_PROMPT = (
        ...
        '| 工具 | 用途 |\n'
        '|---|---|\n'
        '| **run_python_code** | 运行 Python/pandas 做跨表关联/自定义计算/出文件。预置 CSV：work_reports.csv(近90天工单)、work_entries.csv(近90天明细)、zones.csv(全部区域)。用 `pd.read_csv()` 加载。生成文件保存到当前目录(.xlsx/.csv/.json/.txt)，自动提供给用户下载 |\n'
        '\n'
        '## 选工具的决策规则\n'
